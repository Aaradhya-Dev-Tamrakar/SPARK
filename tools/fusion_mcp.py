...
def create_sphere(radius, x=0.0, y=0.0, z=0.0):
    script = (
        ...
        "        adsk.core.Point3D.create({x}, {y}, {z}), adsk.core.Point3D.create({xr}, {y}, {z}), math.pi)\n"
        "    sk.sketchCurves.sketchLines.addByTwoPoints(\n"
        "        adsk.core.Point3D.create({xl}, {y}, {z}), adsk.core.Point3D.create({xr}, {y}, {z}))\n"
        ...
        "    feat = rev.add(inp)\n"
        "    print('Sphere created: radius={r} cm at ({x},{y},{z}), entity=' + feat.name)\n"
    ).format(x=x, y=y, z=z, r=radius, xr=x + radius, xl=x - radius)
