...
**(1) `training/data_prep/prepare_sisfall.py` committed — first code in `training/`.** Parses SisFall's raw per-trial `.txt` files (confirmed structure: 38 subjects, 15 fall types [F01–F15], 19 ADL types [D01–D19], 4506 trial files, 9 columns/row at native 200 Hz) into 6-channel windows matching the proposal's own Sliding Window Buffer spec (200 samples = 2s @ 100 Hz). Downsamples 200→100 Hz via linear interpolation, per the proposal's stated method for this exact dataset. Labels preserved at native F01–15/D01–19 granularity — the binary-vs-multiclass decision is still open (per this session's direct exchange) and is deliberately not collapsed at parse time.

Channel mapping is a documented judgment call, not a proposal-stated fact: SPARK's MPU6050 is a single 6-axis accel+gyro device, but SisFall's raw files contain two separate accelerometers (ADXL345, MMA8451Q) plus one gyroscope (ITG3200) — 9 columns total. The script selects ADXL345 (cols 1–3) + ITG3200 (cols 4–6) as the 6-channel analogue. This is flagged in the script's own docstring for review, not asserted as the proposal's design.

Verified by actually running the script against the real 213 MB SisFall archive (downloaded this session, Kaggle-hosted mirror of the dead `sistemic.udea.edu.co` original — verified as the genuine unmodified dataset, not a third-party pre-windowed derivative): 4500/4500 valid files processed → 38,426 windows (12,586 fall / 25,840 ADL), all 38/38 subjects and all 34/34 activity codes represented. 5 files under `SA15/` internally named `D17_SE15_*` were correctly rejected by the script's own subject-mismatch check — confirmed as exact duplicates of the correctly-filed `SE15/D17_*` originals (upstream SisFall packaging artifact, not a script defect, no data lost from the 38-subject/34-code coverage).

Two Kaggle notebooks surfaced during this same search (`sisfall-wearable-fall-detection-ml.ipynb`, `sisfalldataset.ipynb`) were evaluated and rejected as unsuitable: the first loads an opaque third-party pre-windowed derivative (`nvnikhil0001/sisfall-enhanced`) with unverifiable windowing/labeling; the second has a 10-column schema mismatch against the actual 9-column source files, permanently collapses all fall/ADL codes to binary before any downstream choice is possible, and silently truncates its ADL-file loop partway through via an unexplained `break` at a fixed index. Neither was used as a basis for `prepare_sisfall.py`.

**(2) Process note, logged for traceability, not a project fact.** The commit for item (1) was initially delivered with a plain `git commit`-style message rather than this repo's documented `sync.ps1` workflow (per README/local-workflow convention: routine changes via `.\sync.ps1`, architectural changes via `.\sync.ps1 -m "..."` after a tracker update). Corrected same-session — this v23 entry **is** that tracker update, written before the corrected `sync.ps1 -m` commit is made. No code, BOM, or design content was affected by the process deviation; it was a delivery-format miss only.

**Nothing in §2 (Locked Design), §3 (Team/WBS), §4 (Timeline), or §6 (Pending Benchmark) changed** — this is the first substantive §5-adjacent (Data/Training) content added since the tracker's inception; §5's own text still describes the *target* dataset composition (500+ fall / 2000+ ADL self-collected, still gated on Action #23/HOD confirmation) and is not yet updated to reference the SisFall pretraining pipeline — that cross-link is a follow-up, not done here.
